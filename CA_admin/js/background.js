 ...
 */
 ...
 var render_color={r:255,g:0,b:0};
...
StarField.prototype._renderStarField = function() {
    var i,
        star;
    // Background
   this.canvas.fillStyle = "rgba("+background_color.r+", "+background_color.g+", "+background_color.b+", .5)";
    this.canvas.fillRect(0, 0, this.width, this.height);
    // Stars
    for (i = 0; i < this.numStars; i++) {
        star = this.starField[i];
        this.canvas.fillStyle = "rgba("+(render_color.r)+", "+(render_color.g)+", "+(render_color.b)+", " + star.opacity + ")";
  this.canvas.beginPath();
     this.canvas.arc(star.x + this.width / 2,star.y + this.height / 2, 4, 0, 2 * Math.PI, false);
     this.canvas.fill();
    }
    render_color.r=Math.floor(Math.random()*255)%255;
    render_color.g=Math.floor(Math.random()*255)%255;
    render_color.b=Math.floor(Math.random()*255)%255;
  //  $("body").css("color","rgb("+(render_color.r)+", "+(render_color.g)+", "+(render_color.b) + ")");
};
...
StarField.prototype._watchCanvasSize = function(elapsedTime) {
    ...
    // (Cap to ~3fps)
    if (timeSinceLastCheck >= 1000/30 || !this.prevCheckTime) {
